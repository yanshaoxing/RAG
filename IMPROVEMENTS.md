...
## P1-4 缺 CI、缺 lint/格式化配置

**现状**：无 `.github/workflows/`、无 `pyproject.toml` / `ruff.toml` / `pytest.ini` / pre-commit。测试 2.7 秒跑完且完全离线（不依赖内网和 LLM），是天生适合 CI 的。
...
| 模型 | 输入 | 输出 | 备注 |
|---|---|---|---|
| qwen3.6-flash（主：回答/改写/摘要/图谱抽取） | 1.2 元/M | 7.2 元/M | 缓存命中 0.12 元/M |
| qwen3.5-flash（图谱校验） | 0.2 元/M | 2 元/M | 缓存命中 0.02 元/M |
| text-embedding-v4 | 0.514 元/M | — | Batch File 0.257 元/M |
...
|---|---|---|---|
| ① 分块 | 0 | 0 | 0 |
| ② 摘要树 | ~116 万 | ~16.6 万 | **≈ 2.6 元** |
| ③ BM25 | 0 | 0 | 0 |
| ④ 向量（embedding） | ~52 万 | — | **≈ 0.27 元** |
| ⑤ 图谱（抽取+校验+归一） | ~155 万 | ~29 万 | **≈ 2.9 元** |
| **合计** | | | **≈ 5.8 元** |

中文实际约 0.7 token/字，真实开销约 **4 元**。单次查询约 **0.03 元**，50 条评测题约 **1.5 元**。构建是一次性的，费用不构成约束。

**由单价结构导出的三个优化点**（比"少读几遍原文"重要得多）：

1. **输出比输入贵 6 倍**（7.2 vs 1.2）。所以成本大头是输出量，不是输入量：`SUMMARY_PARENT_RATIO=0.20` / `SUMMARY_CHAPTER_RATIO=0.15` 直接决定摘要输出费用；图谱抽取的 JSON 输出（约 1.15 元）是阶段⑤最贵的单项。想省钱先压输出比例，而不是改 L3 的输入来源。
2. **校验模型比主模型便宜 6 倍**（0.2/2 vs 1.2/7.2）。L1 叶子摘要任务简单、输出短，切到 `qwen3.5-flash` 可省约 0.5 元/全书且质量损失极小 —— 纯配置改动，值得做成 `SUMMARY_LEAF_LLM_PROVIDER`。
3. **显式缓存命中价是输入价的 1/10**（0.12 vs 1.2）。图谱对同一节文本读两遍（抽取 + 校验）、摘要 L2/L3 对同一章原文各读一遍，都是天然的缓存场景。需 DashScope explicit cache 支持，列为后续优化。

> 注：该价目表取自 dashscope-us 公共模型，而本项目实际调用的是 ap-southeast-1 workspace 专属部署，计费口径可能不同。批次 1 的 token 计量做完后，用一次真实账单校准。
...
### 批次 1 · 可观测性（半天）
5. token + 耗时计量（P1-2）：`step_timer` + `DavyLLM` 累计 usage + 入口输出

做完即可用现有第一章语料实测「元/万字」，校准上面的成本表 —— 不需要整书。

### 批次 2 · 防回归（2 小时）
6. GitHub Actions 跑 pytest + `pyproject.toml`（pytest 配置 + ruff）（P1-4）
