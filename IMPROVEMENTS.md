...
**当前进度**：批次 0、1、2、3 已完成；期间连带完成 P0-2、P1-4、P1-5、P1-6、P1-7 及模型迁移。离线单测 **331** 例全过 + `ruff check .` 零告警。最近提交待记，工作区含批次 3 改动。
...
| 批次 3（P1-5） | 三个高风险模块补测试（`staged_indexer` 阶段跳转矩阵 / `graph_retriever` / `cache` 断点续传），+42 例 | 待记 |
...
**下一步**：**批次 4（性能与易用：三路检索并发 P1-1 / `--rebuild <stage>` 命令 P2-3 / 查询级缓存 P2-4 默认关 / 阶段依赖图 P2-6，1 人日）** → 批次 5（评测脚手架，仍不需要整书）。分界线之后（批次 7~9）才需要整书。
