...
**当前进度**：批次 0、1、2、3、4、5、6 已完成；期间连带完成 P0-2、P1-4、P1-5、P1-6、P1-7、P2-1、P2-2、P2-3、P2-4、P2-5、P2-6 及模型迁移。**剩余 P2 小项已清空，分界线之前全部完成。** 离线单测 **373** 例全过 + `ruff check .` 零告警。
...
| 批次 6 | 剩余 P2 小项 + 文档：P2-2 跨书 embedding 诊断（`probe_vector_index` + `--list` 提示）、P2-5（ui 引擎缓存 `max_entries=3` / config `.env` 来源 DEBUG 日志）、README 拆 `docs/`、CLAUDE.md 补 embedding 全局约束 + eval 命令，+4 例 | 待记 |
...
**批次 6 落地细节**：剩余 P2 小项清空 + 文档拆分，离线单测 369 → **373** 例。① **P2-2 跨书 embedding 一致性诊断**：embedding 是**全局硬约束**（不随引擎绑定 —— 向量检索用全局 `Settings.embed_model`，所有书必须共用同一嵌入模型/维度）。加载期 `_check_embed_model_match` 早已按 `embed_model` 精确比对报错（比维度检查更强），批次 6 补的是**提前诊断**：`corpus.probe_vector_index(profile)` 纯读盘（读该书 `data/vector/_DONE.json` 的 `embed_model`/`vector_dim`），不切激活语料、不加载索引，返回 `VectorIndexStatus`（`built`/`embed_model`/`vector_dim`/`matches_current`；旧产物缺字段按「一致」处理，与加载期同策略）；`--list` 据此对模型不一致的书打 ⚠️ 并给出 `--rebuild vector -c <slug> --yes` 修复提示，不必等到查询才炸。CLAUDE.md 写明该全局约束。② **P2-5 健壮性小项**：`ui.py` 的 `@st.cache_resource` → `max_entries=3`（常驻引擎数封顶，书多了内存不无上限增长）；`config._load_env_file` 在 `RAG_DEBUG` 下打印每个 `.env` 键的**来源**（`.env` 新设 vs 已有 env 变量压过 —— 排查密钥问题的常见坑），**只印键名与来源、绝不印值**，走 stderr（import 期日志系统未配置）。③ **README 拆 `docs/`**：676 行 README 拆成 `docs/architecture.md`（整体架构 + 关键设计 + 目录结构）/ `docs/operations.md`（环境/安装/使用/多书/重建/限制）/ `docs/models.md`（模型配置），README 只留导航表 + 功能特性 + 快速开始（用脚本按行切片保原样，未重打巨型 mermaid/ASCII 图）。④ **CLAUDE.md 补命令**：eval 脚手架三条命令 + 命中判定按内容锚点的设计说明。测试：`tests/test_corpus.py` +4 例（未构建/一致/不一致/旧产物无模型字段）。**P2-5 表中 `_gap_filter` score-None、config provider 环境变量化在批次 0 已完成；P2-1 CLI 显式传 slug 亦在批次 0 完成。**

**下一步**：**批次 7（需要整书）**。分界线之前的所有项已全部完成。批次 7~9 需整书：全本入库（只跑阶段①算真实预算）→ `*_MIN_SCORE`/`GAP`/`RRF` 标定（复用批次 5 评测脚手架）→ 消融实验（摘要树/图谱增量收益）→ `data/` 存储策略（git-lfs / 一键重建）。
...
## P1-1 三路检索是串行的，白白多等一次 embedding 往返 —— ✅ 已完成（批次 4）

**已落地**：`_single_query_retrieve` 三路（向量NL/向量HyDE/BM25）包成闭包任务，`run_parallel_captured(max_workers=config.RETRIEVAL_ROUTE_MAX_CONCURRENCY=3)` 并发；BM25 的 `original_text` 还原搬进该路任务。日志由该工具按提交序回放保序。

**原始现状**：`rag/retrieval/hybrid_retriever.py:88-95`
...
## P1-5 核心装配路径与图谱运行时无单测覆盖 —— ✅ 三个高风险模块已完成（批次 3）

**已落地**：`tests/test_staged_indexer.py`（新增 `TestStageTransitionMatrix`，阶段跳转矩阵）、`tests/test_graph_retriever.py`、`tests/test_cache.py`，合计 +42 例（详见「下次继续 · 批次 3 落地细节」）。**其余模块**（bootstrap / query_engine 的非流式部分 / extractor / merger / schema / text / logging_utils）**仍无专门测试**，风险较低，暂缓。
...
## P2-1 CLI 的 `--corpus` 走的是隐式全局状态 —— ✅ 已完成（批次 0）

**已落地**：`app/cli.py` 用 `corpus.load_profile(args.corpus)` 只校验档案（不改全局态），`build_engine(args.corpus)` 显式把 slug 传进 `build_query_engine`，切换统一在构建锁内完成，与 UI 路径一致。

**原始现状**：`app/cli.py:136-148` 先 `corpus.set_active_corpus(args.corpus)` 切全局激活语料，然后 `build_engine()` 不带参数调用；而 `build_query_engine(corpus_slug)` 本来就支持显式传 slug（`bootstrap.py:57`）。功能正确，但依赖"前面已经切过全局态"这一隐式前提，与 UI 的显式传参路径（`ui.py:52`）不一致。
...
## P2-2 多书共享同一个全局 embedding 模型，这是硬约束但没写下来 —— ✅ 已完成（约束加载期校验在模型迁移时落地；跨书提前诊断 + 文档在批次 6）

**已落地**：加载期 `staged_indexer._check_embed_model_match` 按 `embed_model` 精确报错（模型迁移时落地，比维度检查更强）；批次 6 补 `corpus.probe_vector_index()` 跨书提前诊断 + `--list` ⚠️ 提示 + CLAUDE.md 写明「embedding 全局硬约束」。详见「下次继续 · 批次 6 落地细节」。
...
## P2-3 重建阶段只能手工 `rm -rf` —— ✅ 已完成（批次 4，与 P2-6 同处改动）

**已落地**：`python -m app.cli --rebuild {chunks,summary,bm25,vector,graph} [-c slug] [--yes]`。内部走 `staged_indexer.plan_rebuild`（按依赖闭包，非线性区间）打印将删阶段清单，`--yes` 才真删；不带 `--yes` 仅预览。删除后下次运行入口自动重建。

**原始现状**：README/CLAUDE.md 里重建索引的操作是"删掉对应目录再跑入口"。容易删错语料（路径含 slug）、也容易在还没想清楚时误删下游阶段。
...
## P2-4 没有查询级缓存 —— ✅ 已完成（批次 4）

**已落地**：`HybridRetriever._retrieve` 外层实例级 LRU，key = (语料 slug〔构建期绑定〕 + 原始 query + 检索 config 指纹)，只缓存检索结果、不缓存回答（回答仍流式）。`QUERY_CACHE_ENABLED` 默认关、`QUERY_CACHE_MAX_SIZE` 容量上限；指纹只含影响检索结果的 config 项，任一变化即失效。

**原始现状**：完全相同的问题重复提问会重跑整条管线（3 次改写 LLM + 2 次 embedding + rerank + 图谱抽实体 + 回答）。演示和评测场景下这是纯浪费。
...
## P2-5 若干健壮性小口子 —— ✅ 全部完成（批次 0 + 批次 6）

| 位置 | 问题 | 处置 | 批次 |
|---|---|---|---|
| `hybrid_retriever.py:300` | `n.score >= min_score`，`score` 为 `None` 时抛 `TypeError` | ✅ 改成 `(n.score or 0.0) >= min_score` | 批次 0 |
| `config.py` provider 开关 | `ANSWER_PROVIDER` 等只能改代码 | ✅ `_env_str`/`_env_bool` 统一环境变量覆盖（7 provider + 8 开关） | 批次 0 |
| `config.py:21-38` | `.env` 加载用 `setdefault`，已存在的环境变量静默压过 `.env` | ✅ `RAG_DEBUG` 下打印每键来源（`.env` / 已有 env 变量），只印键名不印值，走 stderr | 批次 6 |
| `ui.py:48-52` | `@st.cache_resource` 按 slug 缓存引擎，书多了内存无上限增长 | ✅ `@st.cache_resource(max_entries=3)` | 批次 6 |
| `README.md` | 单文件架构图 / 操作 / 模型全挤在一起 | ✅ 拆 `docs/{architecture,operations,models}.md`，README 只留导航 | 批次 6 |
...
## P2-6 阶段依赖被写死成线性链，图谱被无谓连带重建 —— ✅ 已完成（批次 4）

**已落地**：阶段表从有序列表改成带依赖声明的 `_Stage` 图（`staged_indexer._STAGES`：`chunks→summary→{bm25,vector}`，`graph→(raw)`）。`get_or_build_index` 改用 `_compute_dirty` 沿依赖边闭包传播重建集，删除/重建/加载均按闭包判定：**删向量不再连带删图谱**，bm25 与 vector 互不牵连。连带修掉旧「全完成」分支无条件 `load_graph()`（禁用图谱时也不再误加载）与无谓加载 chunks。阶段跳转矩阵测试已重写为闭包语义。

**原始现状**：`staged_indexer.py` 的阶段表是有序列表，重建时「删除 start_from 及之后所有启用阶段的目录」。但**图谱阶段（⑤）的输入是 `raw/` 原文**（`build_graph(load_documents())`，按节抽取，独立于检索分块），它并不依赖分块/摘要/BM25/向量中的任何一个。
...
### 批次 3 · 给高风险代码补测试（1 人日）（P1-5）—— ✅ 已完成（331 单测，+42）
7. ✅ `staged_indexer` 阶段跳转矩阵（桩掉全部 build/load 分支 + `_stage_complete`，断言删第 N 阶段的 rebuild/load 集合、删除范围、开关跳过）
8. ✅ `graph_retriever`（假 LLM + 假图谱存储：可用性短路 / 参数化防注入 / 去重 / 单实体异常隔离 / 限额）
9. ✅ `cache.py` 断点续传（`processing` 不算完成 / 文本变更重跑 / 跨进程重开恢复 / 字段无损往返 / 归一化 / 描述合并取最长）

批次 4 的改动与批次 7 的长跑构建的安全带，必须排在它们之前。

### 批次 4 · 性能与易用（1 人日）—— ✅ 已完成（346 单测，+15）
10. ✅ 三路检索并发（P1-1）：`run_parallel_captured(max_workers=3)`，BM25 还原逻辑随任务搬入
11. ✅ `--rebuild <stage>` 命令（P2-3）：按依赖闭包打印删除清单，`--yes` 才真删
12. ✅ 查询级缓存（P2-4，默认关）：实例级 LRU，key=slug+query+检索指纹，只缓存检索结果
13. ~~embedding 维度校验~~ → 已在模型迁移时提前完成（升级为**模型名**校验，见 P2-6）
14. ✅ **阶段依赖图**（P2-6）：`_STAGES` 依赖图 + `_compute_dirty` 闭包传播，删向量不再连带删图谱、bm25/向量互不牵连
...
### 批次 6 · 剩余 P2 小项 + 文档 —— ✅ 已完成（373 单测，+4）
18. ✅ P2-2 跨书 embedding 诊断（`probe_vector_index` + `--list` ⚠️ 提示）
19. ✅ P2-5：`ui.py` 引擎缓存 `max_entries=3` + config `.env` 来源 DEBUG 日志
20. ✅ README 拆 `docs/{architecture,operations,models}.md`（README 只留导航）
21. ✅ CLAUDE.md 补 embedding 全局约束 + eval 脚手架命令
