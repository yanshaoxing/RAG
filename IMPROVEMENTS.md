...
**当前进度**：批次 0、1 已完成；期间连带完成 P0-2、P1-6、P1-7 及模型迁移。离线单测 **289** 例全过。最近提交 `a05d322`，工作区干净。
...
**下一步**：**批次 2（CI + ruff，约 2 小时）** → 批次 3（补 `staged_indexer` 阶段跳转矩阵 / `graph_retriever` / `cache` 测试）→ 批次 4 → 批次 5（评测脚手架，仍不需要整书）。分界线之后（批次 7~9）才需要整书。
