...
2. **3-way rewrite** (`rag/retrieval/query_rewriter.py`) — NL rewrite + HyDE (→ vector retrieval) and keyword expansion (→ BM25), all grounded in the novel context block `NOVEL_CONTEXT` in `rag/prompts.py`. Terminology mapping (`assets/terminology.json`) runs even when `REWRITE_ENABLED=False`. The BM25 query string is always passed through `tokenize_for_bm25` (`rag/utils/text.py`) before retrieval — the corpus is jieba-tokenized, and an untokenized Chinese query scores zero.
...
4. **RRF fusion** of the three routes, then **rerank** via bge-reranker-v2-m3 on a vLLM `/v1/rerank` endpoint (`rag/retrieval/reranker.py`). Transient failures (network/429/5xx) get `RERANK_MAX_RETRIES` quick retries; on final failure the pipeline falls back to RRF ordering (`rerank()` returns `None`; scores are never zeroed).
