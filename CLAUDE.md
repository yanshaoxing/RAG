...
# Evaluation scaffolding (eval/<slug>/qa.jsonl; run from repo root)
python scripts/eval_retrieval.py -c WuLingChaShi                          # Recall@k / MRR / 首命中排名，按题型分组
python scripts/eval_retrieval.py -c WuLingChaShi --config-override RERANK_ENABLED=False --save out.json   # 单参数扫描/消融
python scripts/eval_answer.py -c WuLingChaShi                             # 完整问答 + 异模型 LLM-as-judge（忠实度/引用/完整度）
```

Eval design (see `eval/README.md`): gold uses **content anchors** (`gold_chunk_ids` → `chunks.json` order-index, then retrieved chunks are matched back by text containment) not raw node_ids, so labels survive re-chunking/rebuilds. `scripts/eval_common.py` holds the pure (offline-testable) metric/io functions; `--config-override KEY=VALUE` (repeatable, auto-typed) does single-knob sweeps by `setattr`-ing `config` before build. The tiny demo corpora have limited discrimination (both Recall@10 = 1.00) — real `*_MIN_SCORE`/`GAP`/`RRF` calibration waits for the full book, reusing this scaffolding.

### Rebuilding indexes

Indexing is staged; each stage persists to its own folder under the active corpus's `corpora/<slug>/data/` and writes a `_DONE.json` completion marker when finished. Stage-completion detection uses the marker (a directory without one is treated as an interrupted build and gets rebuilt). Stages form a **dependency graph, not a linear chain** (`staged_indexer._STAGES`): `chunks → summary → {bm25, vector}`, and `graph → (raw)` — the graph stage reads `raw/` directly and is **independent** of the retrieval stages, so rebuilding vector no longer drags graph along (`_compute_dirty` propagates rebuild along dependency edges via closure; bm25 and vector are siblings and don't force-rebuild each other). To rebuild a stage, either **delete its folder** and re-run an entry point, or use `python -m app.cli --rebuild {chunks,summary,bm25,vector,graph} [-c slug]` (previews the deletion closure; `--yes` to actually delete). Downstream stages in the closure rebuild too; untouched stages are loaded from disk:
...
Three providers, switchable per role via config: `"ollama"` (local/remote Ollama), `"davy"` (Lenovo internal OpenAI-compatible cloud endpoint), and `"aliyun"` (public Aliyun workspace, the current default — reuses the `DavyLLM` client with `cert_path=""` meaning system CA). `DavyLLM` implements llama_index `CustomLLM`, uses the CA cert in `assets/` for Davy (custom-CA verify), strips `<thinking>`/`<think>` blocks from responses — incrementally via `ThinkStreamFilter` in `stream_chat`, which is true token streaming (per-SSE-chunk yield) — and retries 429/5xx/network errors with exponential backoff honoring `Retry-After` — `DAVY_MAX_RETRIES`. Answer synthesis is **truly token-streaming end-to-end** when `ANSWER_STREAM_ENABLED=True`: `GraphAugmentedQueryEngine` overrides `_query`+`synthesize` to bypass llama_index's Refine synthesizer (whose `DefaultRefineProgram.stream_call` accumulates the whole stream and yields once) — it fills `QA_TEMPLATE` with the reranked nodes and calls `Settings.llm.stream_chat()` directly, yielding per token; non-streaming still uses the parent's compact synthesizer. Note the parent `_query` calls `self._response_synthesizer.synthesize` directly (bypassing `self.synthesize`), so `_query` is overridden too. `DavyLLM.metadata` declares a truthful `context_window`/`num_output` (`ALIYUN_CONTEXT_WINDOW=1M`/`ALIYUN_NUM_OUTPUT`, from `LLM.txt`); llama_index's 3900-token default would repack a full-book context into several chunks and trigger multi-round refine (extra LLM calls, information loss). This is why single-block compact ≡ one QA call, which is what the streaming path relies on. Four factory functions create role-specific LLMs: answer, rewrite, summary, and graph-validation (the validator deliberately uses a *different* model than extraction for cross-checking: `qwen-flash` vs `qwen3.5-flash`). Embeddings switch on `EMBED_PROVIDER`: aliyun `qwen3.7-text-embedding` (1024-dim, `OpenAILikeEmbedding`, batch ≤10 per DashScope limit) or remote Ollama `qwen3-embedding:8b` (4096-dim).
...
3. **Three-route retrieval + per-route filtering** (`rag/retrieval/hybrid_retriever.py`) — the three routes (vector-NL, vector-HyDE, BM25) run **concurrently** via `run_parallel_captured` (`RETRIEVAL_ROUTE_MAX_CONCURRENCY`, default 3) since the two vector routes each cost an independent embedding round-trip; the BM25 route's `original_text` restoration happens inside its task. Then summary-redundancy filter (drop summary nodes whose covered chunks were already retrieved, `SUMMARY_REDUNDANCY_THRESHOLD`), gap detection (adjacent score-drop ratio), and per-route min-score floors. An optional **query-level cache** (`QUERY_CACHE_ENABLED`, default off) wraps `_retrieve`: key = (corpus slug bound at construction + original query + retrieval-config fingerprint), an instance-level LRU (`QUERY_CACHE_MAX_SIZE`) caching only the retrieval result, not the answer (which still streams).
...
### Usage metering: `rag/metering.py`

Token counts come **only** from the provider's `usage` field in the response (same numbers used for billing) — never from local tokenization. All four paths are wired: chat non-streaming (`DavyLLM._call`), chat streaming (the trailing usage chunk with `choices: []`, previously swallowed by the parser's except branch), rerank (`usage.total_tokens`), and embedding — the last needs `rag/llm/embedding.py`'s `MeteredOpenAILikeEmbedding` because llama_index's `get_embeddings()` throws the usage away. `reasoning_tokens` is tracked separately: it is billed as output but never reaches `message.content`, so the `<think>` stripper cannot see it. Calls that return no usage (Ollama) are counted as `unmetered_calls` and flagged in the summary — estimated numbers must never be passed off as measured ones; models missing from `config.MODEL_PRICES` make `total_cost` `None` rather than silently under-reporting. `capture_usage()` mirrors `capture_pipeline_logs()` (contextvars); `run_parallel_captured` propagates the context via `contextvars.copy_context()` so worker-thread calls (3-way rewrite, sub-queries) accumulate into the same `Meter` — without that, most query-time tokens would be missed.
