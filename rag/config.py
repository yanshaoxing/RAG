...
def _load_env_file() -> None:
    ...
    注意 setdefault 的语义：已存在的（可能是过期的）同名环境变量会静默压过 .env。
    排查密钥问题时最易被这一点坑，故 RAG_DEBUG 下打印每个键的来源（只印键名与来源、
    绝不印值）。此函数在 import 期运行，日志系统尚未配置，故走 stderr。
    """
    env_path = os.path.join(os.path.dirname(__file__), "..", ".env")
    if not os.path.exists(env_path):
        return
    debug = os.environ.get("RAG_DEBUG", "").strip().lower() in ("1", "true", "yes", "on")
    with open(env_path, encoding="utf-8") as f:
        for line in f:
            ...
            k, v = line.split("=", 1)
            k = k.strip()
            already_set = k in os.environ
            os.environ.setdefault(k, v.strip())
            if debug:
                src = "已有 env 变量（.env 被忽略）" if already_set else ".env"
                print(f"[config] 环境变量 {k} 来源：{src}", file=sys.stderr)
